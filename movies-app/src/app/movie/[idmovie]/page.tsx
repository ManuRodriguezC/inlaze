...
import { Movie, Person, Review } from '@/app/components/Types';
...
export default function MoviePage() {
  ...
  useEffect(() => {
    if (!idmovie || typeof idmovie !== 'string') {
      return;
    }

    const urlMovie = `https://api.themoviedb.org/3/movie/${idmovie}`;
    const urlCast = `https://api.themoviedb.org/3/movie/${idmovie}/credits`;
    const urlSummary = `https://api.themoviedb.org/3/movie/${idmovie}/reviews`;
    const urlRecomment = `https://api.themoviedb.org/3/movie/${idmovie}/similar`;
  ...
  if (!idmovie || typeof idmovie !== 'string') {
    return <div className={styles.loadingMovie}>Movie ID is not available.</div>;
  }
